local file_ignores_for_focused_search = {"%.spec%.[jt]sx?$"}
...
return {
    ...
    config = function()
        ...
        vim.keymap.set('n', '<leader>ff', function() -- focused search (non-essential files are ignores)
            builtin.find_files({
                file_ignore_patterns = file_ignores_for_focused_search
            })
        end, {
            desc = 'Find files'
        })
        vim.keymap.set('n', '<leader>faf',
            function() -- sind ALL files (no ignores except build, dist, node_modules etc)
                builtin.find_files({
                    hidden = true
                })
            end, {
